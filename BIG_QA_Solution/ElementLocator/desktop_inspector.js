...
(function() {
    ...
    // ─── PO Sandbox: Simulate a single PO method step ────────────────────────
    // Called by locator_studio.py when user clicks "Simulate All Methods" in POWindow.
    // actionType: 'highlight' | 'fill' | 'click'
    // Returns: { found: bool, tag: str, action: str }
    window.simulatePOStep = function(actionType, locType, locValue, fillArg) {
        // Inject sandbox highlight style if not present
        if (!document.getElementById('po-sandbox-style')) {
            const s = document.createElement('style');
            s.id = 'po-sandbox-style';
            s.innerHTML = `
                @keyframes po-sandbox-glow {
                    0%   { box-shadow: 0 0 0 3px rgba(0,188,242,0.9), 0 0 20px rgba(0,188,242,0.5); }
                    50%  { box-shadow: 0 0 0 6px rgba(0,188,242,0.5), 0 0 35px rgba(0,188,242,0.3); }
                    100% { box-shadow: 0 0 0 3px rgba(0,188,242,0.9), 0 0 20px rgba(0,188,242,0.5); }
                }
                .po-sandbox-highlight {
                    outline: 2.5px solid #00bcf2 !important;
                    outline-offset: 2px !important;
                    animation: po-sandbox-glow 1s ease-in-out 2 !important;
                    transition: outline 0.15s ease !important;
                }
                .po-sandbox-fill {
                    outline: 2.5px solid #10b981 !important;
                    outline-offset: 2px !important;
                    box-shadow: 0 0 0 3px rgba(16,185,129,0.4), 0 0 20px rgba(16,185,129,0.25) !important;
                }
                .po-sandbox-click {
                    outline: 2.5px solid #f59e0b !important;
                    outline-offset: 2px !important;
                    box-shadow: 0 0 0 3px rgba(245,158,11,0.5), 0 0 20px rgba(245,158,11,0.3) !important;
                }
            `;
            document.head.appendChild(s);
        }

        // Clear previous sandbox highlights
        document.querySelectorAll('.po-sandbox-highlight,.po-sandbox-fill,.po-sandbox-click')
            .forEach(el => {
                el.classList.remove('po-sandbox-highlight', 'po-sandbox-fill', 'po-sandbox-click');
                el.style.outline = '';
                el.style.boxShadow = '';
            });

        // Resolve element using existing _doVerify infrastructure
        let el = null;
        try {
            const uType = (locType || '').toUpperCase().replace(/\s/g, '');
            if (uType === 'ID') {
                el = document.getElementById(locValue);
            } else if (uType === 'CSS' || uType === 'NAME') {
                el = document.querySelector(uType === 'NAME' ? `[name="${locValue}"]` : locValue);
            } else if (uType === 'XPATH') {
                const r = document.evaluate(locValue, document, null,
                    XPathResult.FIRST_ORDERED_NODE_TYPE, null);
                el = r.singleNodeValue;
            } else if (uType === 'GETBYLABEL') {
                el = document.querySelector(`[aria-label="${locValue}"]`) ||
                     (() => {
                         const labels = Array.from(document.querySelectorAll('label'));
                         const lbl = labels.find(l => l.textContent.trim() === locValue);
                         return lbl ? document.getElementById(lbl.htmlFor) : null;
                     })();
            } else if (uType === 'GETBYPLACEHOLDER') {
                el = document.querySelector(`[placeholder="${locValue}"]`);
            } else if (uType === 'GETBYTEXT') {
                el = Array.from(document.querySelectorAll('*'))
                    .find(n => n.children.length === 0 && n.textContent.trim() === locValue);
            } else if (uType === 'GETBYTESTID') {
                el = document.querySelector(`[data-testid="${locValue}"],[data-test="${locValue}"]`);
            } else if (uType === 'GETBYROLE') {
                const tagMap = { link:'a', checkbox:'input[type="checkbox"]',
                    textbox:'input[type="text"],textarea', button:'button,[role="button"]',
                    heading:'h1,h2,h3,h4,h5,h6', img:'img', list:'ul,ol', listitem:'li' };
                const candidates = Array.from(document.querySelectorAll(tagMap[locValue] || `[role="${locValue}"]`));
                el = candidates[0] || null;
            } else {
                // Fallback: try CSS, then XPath
                try { el = document.querySelector(locValue); } catch(e) {
                    try {
                        const r = document.evaluate(locValue, document, null,
                            XPathResult.FIRST_ORDERED_NODE_TYPE, null);
                        el = r.singleNodeValue;
        ...
        if (!el) return JSON.stringify({ found: false, tag: null, action: actionType });

        // Scroll into view
        try { el.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch(e) {}

        const tag = el.tagName.toLowerCase();
        let applied = actionType;
        ...
        if (actionType === 'fill' && fillArg) {
            ...
            const typeNext = () => {
                if (i < chars.length) {
                    ...
                    setTimeout(() => el.classList.remove('po-sandbox-fill'), 1200);
                }
            };
            typeNext();
        } else if (actionType === 'click') {
            el.classList.add('po-sandbox-click');
            setTimeout(() => {
                el.click();
                setTimeout(() => el.classList.remove('po-sandbox-click'), 900);
            }, 300);
        } else {
            // Default: highlight only
            el.classList.add('po-sandbox-highlight');
            setTimeout(() => el.classList.remove('po-sandbox-highlight'), 2000);
            applied = 'highlight';
        }

        return JSON.stringify({ found: true, tag, action: applied });
    };
