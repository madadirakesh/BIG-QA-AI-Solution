...
QA automation framework — **Playwright for Java**, **Cucumber BDD**, **JUnit 5**, **Maven**.
...
- Internet access on first run — Playwright downloads browser binaries (~250 MB) the first
  time you run `mvn exec:java ... CLI install`.
- On Linux, Playwright also needs a handful of shared libraries (libwoff1, etc.). If the
  browsers complain about missing libs, run:
  ```
...
│   │       └── PlaywrightFactory.java         Per-thread Playwright lifecycle
│   └── test/
│       ├── java/
│       │   ├── runners/TestRunner.java        JUnit 5 + Cucumber entry point (Maven Surefire picks this up)
...
3. **Add step definitions** under `src/test/java/stepDefinitions/LoginSteps.java`.
4. Run `mvn test`.
...
- **`'artifactId' with value 'X' does not match a valid id pattern`** — your project name had
  characters Maven disallows. Recreate via the BIG-QA modal; project names are sanitised into
  the artifactId now (e.g. `First Java App` → `first-java-app`).
- **`Property 'APP_URL' not found in .env file`** — you forgot to `cp .env.example .env` and
  edit it.
- **Browsers don't launch on Linux** — install OS-level libs as shown under *Prerequisites*.
- **Tests pass locally but fail in CI** — set `HEADLESS=true` in your CI environment.
